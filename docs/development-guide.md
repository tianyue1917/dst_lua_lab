...
当前 Profile：`algorithm`、`modload`、`frontend`、`server-sim`。后两者分别提供客户端与专用服务器角色 Fixture，仍不等于真实渲染器或服务器进程。
...
```powershell
python dstlab.py module init example_capability
```
...
| `profiles` | 否 | 允许执行该扩展的 Profile；当前为 `algorithm`、`modload`、`frontend`、`server-sim` |
...
`context.config` 当前包含：`profile`、`runtime`、`case_id`、`mod`、`dependencies`、`scripts_zip`、`work_dir`、`report_dir`、`replay_plan`。
...
`pre_mod` Bootstrap 若返回 Lua callable，Worker 会把它保存在 Python 私有队列中，并在目标 `modmain.lua` 返回后单次调用；该 callable 不会写入 `GLOBAL` 或 MOD env。`controlled_replay` 使用这一机制实现不可由目标提前触发的 deferred replay。
...
`controlled_replay` 用显式 JSON 计划回放已经注册的回调。未提供计划时不会加载模块；显式 `replay_plan=[]` 时加载捕获层但绝不执行回调。捕获 Bootstrap 返回 Worker 私有、单次消费的 deferred callback，目标 MOD 无法提前调用或替换 Runner。

支持的 `kind`：

| Kind | 精确匹配字段 | Fixture |
| --- | --- | --- |
| `prefab_postinit` | `target` | 合成实体 |
| `component_postinit` | `target` | 合成组件及 owner |
| `prefab_constructor` | `target` | 无隐式参数 |
| `mod_rpc` | `rpc_type`、`namespace`、`name` | 虚拟玩家或 Shard ID |
| `stategraph_state` | `stategraph`、`state`、`callback` | 有限 StateGraph controller |
...
每次只增加一个计划条目。`strict: true` 会在目标缺失或回调失败时记录 `replay.*` Trace 后停止；非 strict 条目会记录 `skipped` 或 `failed` 并继续。计划内容与详细字段见 [`modules/controlled_replay/README.md`](../modules/controlled_replay/README.md)。

不要把回放成功解释成真实引擎效果，也不要把含本机路径或私有数据的计划提交到仓库。
...
```powershell
python dstlab.py case mount "C:\path\to\external_target"
python dstlab.py case validate external_target --mod "C:\path\to\target-mod"
```

卸载并清理 Lab 内派生文件：

```powershell
python dstlab.py case unmount external_target --purge-generated
```
...
Wheel 安装态把内置扩展作为只读包数据提供；用户状态、报告和新脚手架位于 `%LOCALAPPDATA%\dst-lua-lab`、`$XDG_DATA_HOME/dst-lua-lab` 或显式 `DSTLAB_HOME`，不会隐式信任当前 MOD 目录，也不会写入 `site-packages`。Wheel 命令使用 `dstlab` console entry；源码 Checkout 可使用 `python dstlab.py`。
