...
Frontend 和 Server-Sim 仍是后续能力，不作为当前 CLI 选项提供。
...
省略 `--scripts-zip` 时，`debug-mod` 会在仓库的上一级目录寻找 `scripts.zip`：
...
1. 先检查目标目录是否包含 `modinfo.lua` 和 `modmain.lua`：

```powershell
python dstlab.py inspect-mod --mod "C:\path\to\target-mod"
```

2. 使用通用 Case Pack 启动只读调试：
